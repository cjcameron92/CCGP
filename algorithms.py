...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__, elitism=True):
    ...
    for gen in range(1, ngen + 1):
        ...
        #Elitism:
        #replace random members of population with the members of hof
        if elitism:
            unique_indices = random.sample(range(len(population)), len(halloffame))
            for halloffame_ind, index in zip(halloffame, unique_indices):
                population[index] = halloffame_ind
